...
pub fn iracing() -> Result<(), std::io::Error> {
    ...
    if mmap.is_null() {
        ...
        return Ok(()); // TODO(knielsen): Do something
    }
    info!("The memmap is set up!");

    let header = mmap as *mut irsdk_header;
    
    let event = unsafe { OpenEventW(SYNCHRONIZE, false, event_filename.pwstr()) };
    drop(event_filename);
    info!("Event handle: {:?}, error: {:?}", event, unsafe { GetLastError() });

    let mut buffer = vec![];
    for i in 0..1 {
        match unsafe { WaitForSingleObject(event, u32::MAX) } {
            0x0 => (),
            0x102 => panic!("TIMEOUT!"),
